...
- the node running the clock under test is label with: %s`, pkg.PtpClockUnderTestNodeLabel, pkg.PtpClockUnderTestNodeLabel)
		...
		Context("PTP ClockSync", func() {
			...
			It("Clock state returns to LOCKED after replay-gated reconfiguration (OCPBUGS-85092)", func() {
				...
				By("Waiting for clock to reach LOCKED from live data")
				Eventually(func() error {
					return metrics.CheckClockState(metrics.MetricClockStateLocked, slaveIf, &nodeName)
				}, 5*time.Minute, 10*time.Second).Should(BeNil(),
					"clock should reach LOCKED from live data before replay arrives")

				By("Verifying clock_state stays LOCKED while stale replay arrives")
				Consistently(func() error {
					return metrics.CheckClockState(metrics.MetricClockStateLocked, slaveIf, &nodeName)
				}, 2*time.Minute, 5*time.Second).Should(BeNil(),
					"OCPBUGS-85092: clock_state must stay LOCKED; "+
						"without the live gate, stale FREERUN replay overwrites live LOCKED state")
			})
