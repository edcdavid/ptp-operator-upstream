...
func (r *PtpConfig) validate() error {
	...
	for _, profile := range profiles {
		...
		// [global] has spp -1 which doesn't need to exist in the secret
...
func validateSppPerSection(conf *Ptp4lConf) error {
	for sectionName, section := range conf.sections {
		...
		if sectionName == "[global]" {
			...
			if sppValue != "-1" {
				return fmt.Errorf("spp in [global] section must be -1 to allow unauthenticated UDS communication (phc2sys/pmc), got spp %s. Set spp -1 in [global] and configure spp per-interface", sppValue)
			}
		} else {
			// Interface sections: spp must NOT be -1 (should enable auth on network interfaces)
			if sppValue == "-1" {
				return fmt.Errorf("spp in interface section %s must not be -1 (auth should be enabled on network interfaces), got spp -1", sectionName)
			}
		}
	}
	return nil
